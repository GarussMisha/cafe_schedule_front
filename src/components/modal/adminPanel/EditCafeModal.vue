  ...
  <div v-if="isOpen" class="modal-overlay" @click.self="closeModal">
    <div class="modal-content">
      ...
      <div v-if="cafe" class="modal-body">
        <div class="edit-label">{{ cafe.name }}</div>
        <div class="form-group"><label>Название</label><input v-model="editCafeData.name" /></div>
        <div class="form-group"><label>Адрес</label><input type="text" v-model="editCafeData.address" /></div>
        <div class="form-group"><label>Телефон</label><input type="text" v-model="editCafeData.phone" /></div>
      </div>

      <div class="modal-actions">
        <button @click="updateCafe" class="btn-primary" type="button" :disabled="isSubmitting">{{ isSubmitting ? 'Обновление...' : 'Обновить' }}</button>
...
const props = defineProps({ isOpen: Boolean, cafe: Object })
const isSubmitting = ref(false)
const editCafeData = ref({ name:'', address:'', phone:'' })
const internalVisible = ref(false)
watch(()=>props.isOpen, v=>{ internalVisible.value=v })
watch(internalVisible, v=>{ if (!v) emit('close') })
watch(()=>props.cafe, c=>{ if (c) editCafeData.value={name:c.name||'',address:c.address||'',phone:c.phone||''} },{immediate:true})
function closeModal() { internalVisible.value=false; emit('close') }
async function updateCafe() {
  if (isSubmitting.value||!props.cafe) return
  if (!editCafeData.value.name.trim()) { toast.add({severity:'warn',summary:'Внимание',detail:'Введите название кафе!',life:3000}); return }
  isSubmitting.value=true
  try { await apiUpdateCafe(props.cafe.id,editCafeData.value); toast.add({severity:'success',summary:'Успех',detail:'Кафе обновлено!',life:3000}); emit('success'); emit('close') }
