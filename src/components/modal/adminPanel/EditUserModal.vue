  ...
  <div v-if="isOpen" class="modal-overlay" @click.self="closeModal">
    <div class="modal-content">
      ...
      <div v-if="user" class="modal-body">
        <div class="edit-label">{{ user.firstName }} {{ user.lastName }} — {{ user.username }}</div>

        <div class="form-group"><label>Логин</label><input v-model="editUserData.username" /></div>
        <div class="form-group"><label>Email</label><input type="email" v-model="editUserData.email" /></div>
        <div class="form-group"><label>Имя</label><input type="text" v-model="editUserData.firstName" /></div>
        <div class="form-group"><label>Фамилия</label><input type="text" v-model="editUserData.lastName" /></div>
        <div class="form-group"><label>Должность</label><input type="text" v-model="editUserData.position" /></div>
        <div class="form-group">
          <label>Роли</label>
          <select v-model="editUserData.roles" multiple>
      ...
      <div class="modal-actions">
        <button @click="updateUser" class="btn-primary" type="button" :disabled="isSubmitting">{{ isSubmitting ? 'Обновление...' : 'Обновить' }}</button>
...
const editUserData = ref({ username:'', email:'', firstName:'', lastName:'', position:'', roles:[] })
const roleOptions = [{label:'STAFF',value:'STAFF'},{label:'CAFE_ADMIN',value:'CAFE_ADMIN'},{label:'USER_ADMIN',value:'USER_ADMIN'}]
const internalVisible = ref(false)
watch(()=>props.isOpen, v=>{ internalVisible.value=v })
watch(internalVisible, v=>{ if (!v) emit('close') })
watch(()=>props.user, u=>{ if (u) editUserData.value={username:u.username,email:u.email,firstName:u.firstName||'',lastName:u.lastName||'',position:u.position||'',roles:[...(u.roles||[])]} },{immediate:true})
function closeModal() { internalVisible.value=false; emit('close') }
async function updateUser() {
  if (isSubmitting.value||!props.user) return
  isSubmitting.value=true
  try { await userStore.updatedUser_store(props.user.id,editUserData.value); toast.add({severity:'success',summary:'Успех',detail:'Пользователь обновлён!',life:3000}); emit('success'); emit('close') }
  catch(e) { toast.add({severity:'error',summary:'Ошибка',detail:e.response?.data?.message||'Ошибка обновления',life:5000}) }
...
.edit-label { font-size:14px; font-weight:600; color:var(--color-primary); margin-bottom:14px; padding-bottom:10px; border-bottom:1px solid var(--color-divider-soft); }
...
.form-group small { font-size:11px; color:var(--color-ink-muted-48); margin-top:4px; }
